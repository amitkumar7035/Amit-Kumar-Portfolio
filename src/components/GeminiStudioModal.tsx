...
interface GeminiStudioModalProps {
  isOpen: boolean;
  onClose: () => void;
  defaultTab?: 'chat' | 'maps' | 'image' | 'music' | 'video';
}
...
export const GeminiStudioModal: React.FC<GeminiStudioModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'chat' | 'maps' | 'image' | 'music' | 'video'>(defaultTab);
  ...
  // GROUNDING SELECTION & USER LOCATION
  ...
  const handleDetectLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        setIsLocating(false);
        setLocationStatus(`Location locked (${pos.coords.latitude.toFixed(2)}, ${pos.coords.longitude.toFixed(2)})`);
      },
      (err) => {
        setIsLocating(false);
        setLocationStatus('Could not access GPS. Defaulting to regional routing.');
      },
      { timeout: 8000 }
    );
  };
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: 'welcome',
      role: 'assistant',
      content: `Hello! I'm **Amit's Gemini AI Copilot**, powered by **Google Gemini 3.5** with live **Google Search Grounding** and **Google Maps Grounding**.\n\nAsk me about Amit's projects, tech stack, coding profiles, or ask questions grounded in real-time web search or Google Maps location data!`,
      timestamp: 'Just now',
    },
  ]);
  ...
  const handleSendChat = async (e?: React.FormEvent, promptOverride?: string) => {
    ...
    try {
      ...
      const botMsg: ChatMessage = {
        ...
        places: data.places || [],
        activeTool: data.activeTool || 'none',
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      };
  ...
  // DEDICATED GOOGLE MAPS GROUNDING HANDLER
  const handleSearchMaps = async (e?: React.FormEvent, queryOverride?: string) => {
    if (e) e.preventDefault();
    const queryToSend = queryOverride || mapsQuery.trim();
    if (!queryToSend || isMapsLoading) return;
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        throw new Error(data.error || 'Failed to query Google Maps');
      }
      ...
      setMapsError(err.message || 'Error communicating with Google Maps grounding.');
    } finally {
      ...
      >
        ...
        {activeTab === 'chat' && (
          <div className="flex-1 flex flex-col min-h-0">
                  ...
                  <div className="space-y-1.5">
                    ...
                    {msg.sources && msg.sources.length > 0 && (
                      <div className="pt-1 flex flex-wrap gap-1.5 items-center">
                        <span className="inline-flex items-center gap-1 text-[10px] font-mono text-cyan-500 font-semibold">
                          <Globe className="w-3 h-3" /> Google Search Sources:
                        </span>
                        {msg.sources.map((src, idx) => (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-md bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-600 dark:text-cyan-400 transition-colors border border-cyan-500/20 truncate max-w-[200px]"
                          >
                    ...
                    {msg.places && msg.places.length > 0 && (
                      <div className="pt-2 space-y-2 max-w-full">
                        <div className="inline-flex items-center gap-1.5 text-[11px] font-mono font-semibold text-emerald-500 dark:text-emerald-400">
                          <MapPin className="w-3.5 h-3.5" />
                          <span>Google Maps Grounded Locations ({msg.places.length}):</span>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          {msg.places.map((place, pIdx) => (
                            <div
                              key={pIdx}
                              className="p-3 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs flex flex-col justify-between gap-2 shadow-xs"
                            >
                              <div>
                                <div className="font-semibold text-slate-900 dark:text-white flex items-start justify-between gap-2">
                                  <span className="truncate">{place.title}</span>
                                  <a
                                    href={place.uri}
                                    target="_blank"
                                    rel="noreferrer"
                                    className="text-cyan-500 hover:text-cyan-400 p-0.5 rounded hover:bg-cyan-500/10 transition-colors shrink-0"
                                ...
                                {place.reviews && place.reviews.length > 0 && (
                                  <div className="mt-2 text-[10px] text-slate-600 dark:text-slate-400 italic bg-white dark:bg-slate-950 p-2 rounded-lg border border-slate-200/60 dark:border-slate-800/80 line-clamp-2">
                              ...
                              <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="inline-flex items-center justify-center gap-1.5 w-full py-1.5 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 font-semibold text-[11px] border border-emerald-500/30 transition-colors"
                              >
              ...
              {isChatLoading && (
                <div className="flex gap-3 max-w-[80%] mr-auto">
                  ...
                  <div className="p-3.5 rounded-2xl bg-slate-100 dark:bg-slate-800 text-slate-500 rounded-tl-none flex items-center gap-2 text-xs">
                    ...
                    <span>
                      {groundingMode === 'maps'
                        ? 'Querying Google Maps data with gemini-3.5-flash...'
                        : 'Synthesizing with Google Grounding (gemini-3.5-flash)...'}
                    </span>
                  </div>
                </div>
              )}
            ...
            {/* Grounding Mode & Location Control Bar */}
            <div className="px-4 py-2 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/60 flex items-center justify-between gap-2 flex-wrap text-xs">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  disabled={isLocating}
                  className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:text-cyan-500 border border-slate-200 dark:border-slate-700 text-xs transition-colors"
                >
                  {isLocating ? (
                  ...
                  <span>{userLocation ? 'GPS Locked' : 'Detect Location'}</span>
                </button>
                {locationStatus && (
                  <span className="text-[10px] font-mono text-slate-400 hidden md:inline truncate max-w-[140px]">
                    {locationStatus}
                  </span>
                )}
              </div>
            ...
            >
              <input
                ...
                placeholder={
                  groundingMode === 'maps'
                    ? 'Search locations, IT parks, colleges, tech hubs with Google Maps...'
                    : 'Ask about Amit, web dev, or any tech query with Google Search grounding...'
                }
        ...
        {/* TAB: GOOGLE MAPS GROUNDING (gemini-3.5-flash with googleMaps tool) */}
        {activeTab === 'maps' && (
            ...
            <div className="max-w-3xl mx-auto space-y-5">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/25">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-600 dark:text-slate-400 mt-0.5">
                      Ask about tech hubs, campuses, companies, and developer spots grounded in real-time Google Maps data.
                    </p>
                  </div>
                </div>
                ...
                <button
                  ...
                  onClick={handleDetectLocation}
                  disabled={isLocating}
                  className="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-xl bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 border border-slate-300 dark:border-slate-700 hover:border-emerald-500 text-xs font-semibold shrink-0 transition-colors cursor-pointer"
                >
                  {isLocating ? (
                  ...
                  <span>{userLocation ? 'GPS Coordinates Locked' : 'Detect My Location'}</span>
                </button>
              </div>

              {/* Search Form */}
              <form onSubmit={handleSearchMaps} className="space-y-3">
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      placeholder="e.g. Find IT parks and software company hubs in Bengaluru..."
                      className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white placeholder:text-slate-400 text-xs sm:text-sm border border-slate-200 dark:border-slate-700 focus:border-emerald-500 focus:outline-none transition-colors"
                  ...
                  <button
                    ...
                    className="px-4 py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold text-xs sm:text-sm flex items-center gap-1.5 disabled:opacity-50 transition-colors shadow-md shadow-emerald-500/20 cursor-pointer"
                  >
                    {isMapsLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Search className="w-4 h-4" />}
                    <span>Explore</span>
                  </button>
                </div>
                ...
                <div className="flex items-center gap-2 overflow-x-auto scrollbar-none pb-1">
                  ...
                  {[
                    'Tech parks in Bengaluru, India',
                    'Top CS engineering colleges in India',
                    'Noida software tech zones',
                    'Developer coworking cafes near me',
                  ].map((preset, idx) => (
                    <button
                      key={idx}
                      type="button"
                      onClick={() => {
                        setMapsQuery(preset);
                        handleSearchMaps(undefined, preset);
                      }}
                      className="px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-emerald-500/10 hover:text-emerald-500 dark:hover:text-emerald-400 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 text-xs whitespace-nowrap transition-colors cursor-pointer"
                    >
                      {preset}
                    </button>
                  ))}
              ...
              {isMapsLoading && (
                <div className="p-8 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-center space-y-3">
                  ...
                  <p className="text-xs sm:text-sm font-medium text-slate-700 dark:text-slate-300">
                    Querying live Google Maps place data with <span className="font-mono text-emerald-400">gemini-3.5-flash</span>...
                  </p>
                  <p className="text-xs text-slate-400">
                    Extracting geocoded coordinates, address details, and visitor review snippets.
                  </p>
                </div>
              )}

              {/* Results View */}
              {mapsResult && !isMapsLoading && (
                <div className="space-y-5 animate-in fade-in duration-200">
                  {/* Synthesis Text Box */}
                  <div className="p-4 sm:p-5 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs sm:text-sm text-slate-800 dark:text-slate-200 leading-relaxed whitespace-pre-wrap">
                    {mapsResult.reply}
                  </div>
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        <h5 className="text-xs font-mono font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
                          ...
                          <span>Google Maps Grounded Locations ({mapsResult.places.length})</span>
                        </h5>
                        <span className="text-[11px] font-mono text-emerald-400">Direct Links Verified</span>
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {mapsResult.places.map((place, idx) => (
                          <div
                            key={idx}
                            className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-between gap-3 hover:border-emerald-500/50 transition-colors"
                          >
                            <div className="space-y-1.5">
                              <div className="flex items-start justify-between gap-2">
                                ...
                                <a
                                  ...
                                  target="_blank"
                                  rel="noreferrer"
                                  className="text-emerald-500 hover:text-emerald-400 p-1 rounded-md hover:bg-emerald-500/10 transition-colors shrink-0"
                              ...
                              {place.reviews && place.reviews.length > 0 && (
                                <div className="mt-2 text-[11px] text-slate-600 dark:text-slate-300 italic bg-slate-50 dark:bg-slate-950 p-2.5 rounded-xl border border-slate-200/70 dark:border-slate-800">
                                  "{place.reviews[0].text}"
                                  {place.reviews[0].author && (
                                    <div className="mt-1 text-[10px] text-slate-400 not-italic font-mono">
                                      — {place.reviews[0].author}
                                    </div>
                                  )}
                                </div>
                            ...
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="inline-flex items-center justify-center gap-1.5 w-full py-2 rounded-xl bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 font-semibold text-xs border border-emerald-500/30 transition-colors shadow-xs"
                            >
